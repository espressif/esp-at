...
static esp_gatt_id_t lock_descr_id;

#define BT_BD_ADDR_STR         "%02x:%02x:%02x:%02x:%02x:%02x"
#define BT_BD_ADDR_HEX(addr)   addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]

static bool connect = false;
static const char device_name[] = "TIDA-00757 ELock";
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_GET_CHAR_EVT:
    {
        ...
        ESP_LOGI(GATTC_TAG, "UUID128: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x\n", p_data->get_char.char_id.uuid.uuid.uuid128[0],
        ...
        if (p_data->get_char.char_id.uuid.uuid.uuid128[12] == 0x71) {
        ...
        memcpy(&lock_descr_id, &(p_data->get_char.char_id), sizeof(lock_descr_id    ));
...
//                ESP_LOGI(GATTC_TAG, "adv: %s, dev: %s\n", adv_name, device_name);            
...
void lock_control(uint8_t lock_state)
{
