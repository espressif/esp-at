...
//#define CONFIG_BT_RESERVE_DRAM 0x5000
...
#define LISTx
        ...
        ESP_LOGI(TAG, "LIST: %s", list);
        rsp.attr_value.len = strnlen(list, 100); //2048-1);
        strncpy((char*)rsp.attr_value.value, list, rsp.attr_value.len);
#else
    ...
    case ESP_GATTS_CONNECT_EVT:
        ESP_LOGI(TAG, "SERVICE_START_EVT, conn_id %d, remote %02x:%02x:%02x:%02x:%02x:%02x:, is_conn %d\n",
