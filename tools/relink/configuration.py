    ...
    def dump(self):
        for libname in self.libs:
            ...
            for objname in lib.objs:
                ...
                print('{}, {}, {}, {}'.format(libname, objname, obj.path, obj.funcs))
...
def generator(library_file, object_file, function_file, sdkconfig_file, objdump='riscv64-unknown-elf-objdump'):
    ...
    for d in csv.DictReader(open(function_file, 'r')):
        if d['option'] and sdkconfig.check(d['option']) == False:
            print('skip {}({})'.format(d['function'], d['option']))
            continue
