...
.. list::
   ...
   - :ref:`PKI 配置 <sysmfg-pki>`：用于存储 SSL/MQTT/HTTPS/WebSocket/WPA2-Enterprise 客户端或服务器所使用的证书和私钥。
...
.. list-table::
   ...
   * - WPA2 Enterprise 客户端
     ...
     -
       ...
       * ``wpa2_ca``
     -
       * AT 作为 WPA2-Enterprise 客户端时，需要配置客户端证书，提供给服务器进行身份验证。只支持单套证书。
       * AT 作为 WPA2-Enterprise 客户端时，需要配置客户端私钥，用于加密客户端与服务器之间的通信。只支持单套私钥。
       * AT 作为 WPA2-Enterprise 客户端时，需要配置客户端 CA 证书，用于校验服务器证书的有效性。只支持单套 CA 证书。
