...
manufacturing NVS 分区包含多个命名空间，每个命名空间包含若干键值对，用于存储不同类型的配置信息。按用途，可分为以下几类，每一类的命名空间、键名以及详细操作示例请参考对应子章节：

.. list::

   - :ref:`出厂参数配置 <sysmfg-factory-param>`：``factory_param`` 命名空间，用于存储模组信息、Wi-Fi 配置和 UART 配置等。
   - :ref:`PKI 配置 <sysmfg-pki>`：用于存储 SSL/MQTT/HTTPS/WebSocket/Wi-Fi Enterprise 客户端或服务器所使用的证书和私钥。
   :not esp32s2: - :ref:`GATTS 配置 <sysmfg-gatts>`：``ble_data`` 命名空间，用于存储低功耗蓝牙 GATTS 服务配置。
   - :ref:`自定义数据存储 <sysmfg-custom-data>`：用户自定义命名空间，用于存储应用特定的持久化数据。
...
- **二进制数据传输**：写入二进制数据（类型 8）时，命令返回 ``>`` 并等待你发送 ``<value>`` 参数中指定的确切字节数。

- **数据持久化**：写入 manufacturing NVS 的数据在设备重启后仍然保留，直到被明确擦除。

- **偏移读取**：你可以使用偏移和长度参数从键中读取部分数据，这对处理大数据块很有用。

- **编译时与运行时**：虽然某些数据（如低功耗蓝牙服务）可以在编译时通过修改源 CSV 文件并重新生成 ``mfg_nvs.bin`` 进行自定义，但 ``AT+SYSMFG`` 命令允许你在运行时更新某些值，无需重新编译。
...
.. list-table::
   ...
   * - ``max_tx_power``
     - Wi-Fi 最大发射功率，取值范围参见 :ref:`AT+RFPOWER <cmd-RFPOWER>` 命令的 ``<wifi_power>`` 参数。
     - :ref:`sysmfg-max-tx-power`
   * - ``country_code``
     - Wi-Fi 国家代码，相关命令：:ref:`AT+CWCOUNTRY <cmd-COUNTRY>`。
   ...
   * - ``uart_rts_pin``
     ...
     - :ref:`sysmfg-uart-config`
   * - ``sys_store``
     - 系统存储模式，控制是否将配置自动保存到 flash，相关命令：:ref:`AT+SYSSTORE <cmd-SYSSTORE>`。
     - :ref:`sysmfg-sys-store`

除上表列出的预定义键名外，你也可以在 ``factory_param`` 命名空间中 :ref:`添加自定义键值对 <sysmfg-custom-key-value>` 来存储应用特定的配置数据。
...
除了预定义的出厂参数外，你还可以在 ``factory_param`` 命名空间中添加自定义键值对来存储应用特定的配置数据。
...
.. list-table::
   ...
   * - SSL 客户端
     ...
     -
       * AT 作为 SSL 客户端时，需要配置客户端证书，提供给服务器进行身份验证。按 ``client_cert.<index>`` 规则存储多套证书。
       * AT 作为 SSL 客户端时，需要配置客户端私钥，用于加密客户端与服务器之间的通信。按 ``client_key.<index>`` 规则存储多套私钥。
       * AT 作为 SSL 客户端时，需要配置客户端 CA 证书，用于校验服务器证书的有效性。按 ``client_ca.<index>`` 规则存储多套 CA 证书。
   * - SSL 服务器
     -
       * ``server_cert``
       * ``server_key``
     ...
     -
       ...
       * ``server_ca``
     -
       * AT 作为 SSL 服务器时，需要配置服务器证书，用于验证客户端的身份。只支持单套证书。
       * AT 作为 SSL 服务器时，需要配置服务器私钥，用于加密服务器与客户端之间的通信。只支持单套私钥。
       * AT 作为 SSL 服务器时，需要配置服务器 CA 证书用于校验客户端证书的有效性。只支持单套 CA 证书。
   * - MQTT 客户端
     -
       * ``mqtt_cert``
       * ``mqtt_key``
     ...
     -
       ...
       * ``mqtt_ca``
     -
       * AT 作为 MQTT 客户端时，需要配置客户端证书，提供给服务器进行身份验证。只支持单套证书。
       * AT 作为 MQTT 客户端时，需要配置客户端私钥，用于加密客户端与服务器之间的通信。只支持单套私钥。
       * AT 作为 MQTT 客户端时，需要配置客户端 CA 证书，用于校验服务器证书的有效性。只支持单套 CA 证书。
   * - HTTPS 客户端
     -
       * ``https_cert``
       * ``https_key``
     ...
     -
       * AT 作为 HTTPS 客户端时，需要配置客户端证书，提供给服务器进行身份验证。按 ``https_cert.<index>`` 规则存储多套证书。
       * AT 作为 HTTPS 客户端时，需要配置客户端私钥，用于加密客户端与服务器之间的通信。按 ``https_key.<index>`` 规则存储多套私钥。
       * AT 作为 HTTPS 客户端时，需要配置客户端 CA 证书，用于校验服务器证书的有效性。按 ``https_ca.<index>`` 规则存储多套 CA 证书。
   * - WebSocket 客户端
     -
       * ``wss_cert``
       * ``wss_key``
     ...
     -
       ...
       * ``wss_ca.0``, ``wss_ca.1``
     -
       * AT 作为 WebSocket 客户端时，需要配置客户端证书，提供给服务器进行身份验证。按 ``wss_cert.<index>`` 规则存储多套证书。
       * AT 作为 WebSocket 客户端时，需要配置客户端私钥，用于加密客户端与服务器之间的通信。按 ``wss_key.<index>`` 规则存储多套私钥。
       * AT 作为 WebSocket 客户端时，需要配置客户端 CA 证书，用于校验服务器证书的有效性。按 ``wss_ca.<index>`` 规则存储多套 CA 证书。
   * - Wi-Fi Enterprise 客户端 (WPA2-Enterprise/WPA3-Enterprise)
     -
       * ``wpa_ent_cert``
       * ``wpa_ent_key``
       * ``wpa_ent_ca``
     -
       ...
       * ``wpa_ent_ca``
     -
       * AT 作为 Wi-Fi Enterprise 客户端时，需要配置客户端证书，提供给服务器进行身份验证。只支持单套证书。
       * AT 作为 Wi-Fi Enterprise 客户端时，需要配置客户端私钥，用于加密客户端与服务器之间的通信。只支持单套私钥。
       * AT 作为 Wi-Fi Enterprise 客户端时，需要配置客户端 CA 证书，用于校验服务器证书的有效性。只支持单套 CA 证书。
       * 上述命名空间和键名由 WPA2-Enterprise 与 WPA3-Enterprise 共用。若使能 ``CONFIG_AT_EAP_LEGACY_NAMESPACE_SUPPORT``，则会改用旧的 ``wpa2_cert``、``wpa2_key``、``wpa2_ca`` 命名以兼容旧固件。

.. note::
  你可以通过 ``AT+SYSMFG?`` 命令查询当前固件支持的所有命名空间，以确认你需要的证书命名空间是否可用。

本节以 SSL 证书为例说明证书配置的完整流程。其他功能的证书配置（如 MQTT、HTTPS）可参照相同方法，仅需根据上表修改对应的命名空间名称和键名即可。
...
1. 读取客户端证书命名空间中的所有键值对：
  ...
  **响应：**
  ...
  同样，你可以查询 client_key 和 client_ca 命名空间：
...
将客户端证书、密钥（双向认证时还需客户端 CA 证书）写入 manufacturing NVS 分区后，你可以使用它们建立安全的 SSL 连接。以下是示例：
...
你可以使用相同的命令擦除 client_key 和 client_ca 数据。
...
1. 读取服务器证书命名空间中的所有键：
  ...
  **响应：**
  ...
  同样，你可以查询 server_key 和 server_ca 命名空间：
...
将服务器证书、密钥（双向认证时还需服务器 CA 证书）写入 manufacturing NVS 分区后，你可以基于这些证书创建 SSL 服务器。以下是一个完整的示例：
...
你可以使用相同的命令擦除 server_key 和 server_ca 数据。
  ...
  **ble_data 命名空间中的键名及说明：**
  ...
  .. list-table::
     ...
     * - 键名
       ...
       - 相关 AT 命令
     * - ``cfg0``, ``cfg1``, ..., ``cfg30``
       - 低功耗蓝牙服务配置项，每个 ``cfg<index>`` 对应 GATT 表中的一行（服务、特征或描述符）。
       -
...
除了预定义的命名空间（如 ``factory_param``、``client_cert``、``server_cert`` 等）之外，你还可以创建自定义命名空间来存储自己的应用数据。这对于存储需要在重启后保留的少量持久化数据非常有用，例如自定义配置、用户偏好设置或设备特定的参数。
...
1. 创建新命名空间并写入数据：
  ...
  **响应：**
  ...
  返回 ``>`` 后，发送你的二进制数据（100 字节）。数据发送完成后，系统返回：

  .. code-block:: none

    OK

  命名空间 ``my_app_ca`` 现已创建，其中包含一个键 ``my_app_ca.0``，存储了你的数据。
...
6. 创建另一个命名空间存储应用配置参数：
  ...
  如果你需要存储配置参数（而非证书数据），建议创建单独的命名空间以保持语义清晰。
