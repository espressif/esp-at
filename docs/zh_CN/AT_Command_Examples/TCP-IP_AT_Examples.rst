...
#. 创建 UDP 传输。分配网络连接 ID 为 4，远程 IP 地址为 ``192.168.3.102``，远端端口为 ``8080``，本地端口为 ``1112``，模式为 ``0``。
   ...
   - ``1112`` 为 {IDF_TARGET_NAME} 设备的 UDP 本地端口，你可自行设置，如不设置则为随机值。
...
本示例中使用的证书是 ESP-AT 中默认的证书，你也可以使用自己的证书：

  * 要使用你自己的 SSL 客户端证书，可以在运行时使用 :ref:`AT+SYSMFG <cmd-SYSMFG>` 命令更新证书（请参考 :ref:`AT+SYSMFG 命令示例中的 PKI 配置 <sysmfg-pki>`），也可以通过重新编译固件替换默认证书（请参考 :doc:`../Compile_and_Develop/How_to_update_pki_config`）。
  * 要使用你自己的 SSL 服务器证书，请用你自己的证书路径替换下面的 SSL 服务器证书。
...
#. 停止发送数据
   ...
   .. Important::
     ...
     使用 ``+++`` 可退出 :term:`透传模式`，回到 :term:`透传接收模式`，此时 TCP 连接仍然有效。你也可以使用 ``AT+CIPSEND`` 命令恢复透传。
...
#. 开启 TCP 服务器。
   ...
   - 设置 TCP 服务器端口为 8080，你也可以设置为其它端口。
