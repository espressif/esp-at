...
本文主要介绍了如何启用日志配置选项。为了获取更多用于调试 ESP-AT 功能的日志信息，请自行 :doc:`编译 ESP-AT 工程 <../Compile_and_Develop/How_to_clone_project_and_compile_it>`，在第三步配置工程里选择 silence mode 为 ``No``。在第五步配置工程里，根据你的实际情况，针对性地启用日志配置选项。
...
.. note::
  ...
  启用日志配置选项后，固件大小将会增加，可能导致编译失败。此时，你可以在 ``python build.py menuconfig`` > ``Component config`` > ``AT`` 路径下禁用不需要的 AT 功能来减小固件大小。
...
^^^^^^^^^^^^^^^^^^^^^^
  ...
  你可以启用以下配置以开启 ICMP 功能 (:ref:`AT+PING <cmd-CIPPING>`) 的调试日志：
...
^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  如果启用以上配置输出的日志仍无法满足你的调试需求，你可以在路径 ``Component config`` > ``LWIP`` > ``Enable LWIP Debug`` 下启用所需的调试配置。
...
.. only:: not esp32s2
  ...
  - 你可以直接在 :term:`AT 日志端口` 查看断开原因码，从而调试 BLE 断开过程。

  .. only:: esp32 or esp32c3

    - 你可以启用以下配置以开启 BLE 扫描、连接、广播、数据收发等交互过程的调试日志：
    ...
    如果以上配置无法满足你的调试需求，你可以在路径 ``Component config`` > ``Bluetooth`` > ``Bluedroid Options`` > ``BT DEBUG LOG LEVEL`` 下启用其他所需的调试配置。

  .. only:: esp32c2 or esp32c5 or esp32c6 or esp32c61

    - 你可以启用以下配置以开启 BLE 功能的调试日志：
...
- 当你想要在 :term:`AT 日志端口` 获取到 AT 通过 TX 发送给 MCU 的数据时，你可开启如下配置：
...
- 当你想要在 :term:`AT 日志端口` 获取 AT 通过 RX 接收到的 MCU 发送数据时，你可开启如下配置：
...
.. only:: not esp32s2
  ...
  场景：ESP 模组作为 BLE 客户端与手机反复建立 BLE 连接和断开连接。
    ...
    - 若你想调试 BLE 断开的原因，请在复测场景时，抓取 :term:`AT 日志端口` 的调试日志进行分析。日志中可能包含断开原因码，这些原因码是 BLE 通用的，并非 AT 特有。示例：

      .. code-block:: none

          Disconnect reason = 0x13

    - 若你想调试 BLE 扫描或连接等过程，请启用 :ref:`AT BLE 功能 <debug-at_ble>` 调试配置，并编译 AT 固件。在复测场景时，抓取 :term:`AT 日志端口` 调试日志进行分析。
