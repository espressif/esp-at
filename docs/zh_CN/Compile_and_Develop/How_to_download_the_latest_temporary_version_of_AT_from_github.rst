...
.. attention::
  **你所下载的最新临时版本 AT 固件仅用于测试目的。若需用于量产，请务必依据你产品的实际应用场景进行全面的功能验证与测试。** 此外，当前固件不支持官方的 OTA 功能 (:ref:`AT+CIUPDATE <cmd-UPDATE>`)。

  **请保存好下载的固件以及下载链接**，用于后续可能的问题调试。

以下步骤指导你如何从 GitHub 下载最新临时版本 AT 固件。

#. 登录你的 GitHub 账号

   在开始之前，**请先登录你的 GitHub 账号**，因为下载固件需要登录权限。
...
#. 点击 "Branch" 选择指定分支
    ...
    默认为 master 分支。如果你想下载指定分支的临时固件，点击 "Branch" 进入指定分支的 workflow 页面。
...
#. 点击下载模组的最新临时版本 AT 固件
   ...
   .. note::
    如果你在 ``Artifacts`` 页面中没有找到对应的模组，请参考 :doc:`esp-at_firmware_differences` 选择类似固件进行下载。
