...
.. important::
  如果你在选择乐鑫硬件产品、AT 软件方案中有任何问题，请联系 `乐鑫商务 <https://www.espressif.com/zh-hans/contact-us/sales-questions>`_ 或者 `技术支持 <https://www.espressif.com/zh-hans/contact-us/technical-inquiries>`_。

硬件选型
------------

在开始使用 ESP-AT 之前，你需要选择一款合适的乐鑫芯片集成到你的产品中，为你的产品赋能无线功能。硬件选型是一个复杂的过程，需要考虑多方面的因素，如功能、功耗、成本、尺寸等。请阅读下面内容帮助你选型硬件。
...
.. list::
  ...
  - `硬件选型入门指导 <https://docs.espressif.com/projects/esp-techpedia/zh_CN/latest/esp-friends/get-started/board-selection.html>`_ 可以帮助你简要对比芯片差别，了解芯片、模组、和开发板的差别以及选择指南。
...
AT 软件方案是乐鑫针对不同芯片提供的 AT 固件，可以帮助你快速实现无线功能。

- 如果你想了解 {IDF_TARGET_NAME} 芯片详细的 AT 软件能力，请参考 :doc:`AT 命令集 <../AT_Command_Set/index>`。
- 如果你想对比 {IDF_TARGET_NAME} 芯片不同的 AT 固件功能，请参考 :doc:`ESP-AT 固件差异 <../Compile_and_Develop/esp-at_firmware_differences>`。
...
.. note::
  出厂的模组或芯片中，均未烧录 AT 固件。若你有量产需求，请及时联系对接的商务人员或 sales@espressif.com，我们将提供定制生产。
...
**官方发布版固件** 又称“发布版固件”、“官方固件”、“默认固件”，为乐鑫官方团队测试通过并发布的固件，固件会根据内部开发计划周期性发布，此种固件可直接基于乐鑫 OTA 服务器升级固件。如果 **官方发布版固件** 完全满足你的项目需求，建议你优先选择 **官方发布版固件**。如果官方固件不支持你的模组，你可以根据 :doc:`硬件差异 <../Compile_and_Develop/esp-at_firmware_differences>`，选择和你的模组硬件配置相近的固件进行测试验证。
...
- 缺点：基于非正式发布的 commit 生成的固件未经过完整的测试，可能会存在一些风险，需要你自己做完整的测试。
...
当你需要进行二次开发时可采用此种方式。需要自己部署 OTA 服务器以支持 OTA 功能。

- 关于如何自行编译固件，请参考 :doc:`../Compile_and_Develop/How_to_clone_project_and_compile_it`。
- 优点：功能、周期自己可控。
- 缺点：需要自己搭建环境编译。

如果你希望稳定性优先，推荐基于该芯片最新已发布的版本对应的分支开发你的 AT 固件。如果你希望更多新功能，推荐基于 `master 分支 <https://github.com/espressif/esp-at/tree/master>`_ 开发你的 AT 固件。

项目初期准备
------------

项目初期准备阶段， **强烈建议** 你选择 `乐鑫开发板 <https://www.espressif.com/zh-hans/products/devkits>`_ 开始你的项目。在项目初期，能够帮助你快速原型验证，评估硬件和软件能力，减少项目风险；在项目中期，能够帮助你快速功能集成和验证，性能优化，提高开发效率；在项目后期，能够帮助你快速模拟和定位问题，实现产品快速迭代。

如果你是 :ref:`self-compiled-firmware`，建议你优先选择 Linux 系统作为开发环境。
