...
2. `./build.py menuconfig` -> `Component config` -> `AT` -> `AT ethernet support` to enable ethernet.
3. `./build.py menuconfig` -> `Component config` -> `AT` -> `Ethernet PHY` to choose ethernet.
