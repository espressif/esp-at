...
RAW_DATA_FOLDER=$PROJECT_PATH/components/customized_partitions/raw_data/server_cert

RAW_DATA_FILE=$(ls $RAW_DATA_FOLDER/*.pem $RAW_DATA_FOLDER/*.der $RAW_DATA_FOLDER/*.cer $RAW_DATA_FOLDER/*.crt 2>/dev/null | awk '{print $1}')

echo "generating server_cert.bin: python $TOOL_PATH generate_bin -b $TARGET_FOLDER/$PARTITION_NAME cert $RAW_DATA_FILE"
