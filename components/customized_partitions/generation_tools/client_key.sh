...
RAW_DATA_FOLDER=$PROJECT_PATH/components/customized_partitions/raw_data/client_key

RAW_DATA_FILE=$(ls $RAW_DATA_FOLDER/*.pem $RAW_DATA_FOLDER/*.der $RAW_DATA_FOLDER/*.key 2>/dev/null | awk '{print $1}')

echo "generating client_key.bin: python $TOOL_PATH generate_bin -b $TARGET_FOLDER/$PARTITION_NAME key $RAW_DATA_FILE"
